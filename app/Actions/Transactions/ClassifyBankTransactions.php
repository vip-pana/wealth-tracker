...
class ClassifyBankTransactions extends Action
{
    ...
    {
        $markers = self::TRANSFER_MARKERS;

        $extra = config('transactions.transfer_markers');
        if (is_array($extra)) {
            $markers = array_merge($markers, array_values(array_filter($extra, is_string(...))));
        }

        foreach ($markers as $marker) {
            if ($marker !== '' && stripos($note, $marker) !== false) {
                return true;
            }
        }
