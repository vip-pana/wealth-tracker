...
// ─── Auth (guest-only) ────────────────────────────────────────────────────────
// Single user: no registration, no password reset. `guest` keeps an authed
// session out; the `setup` middleware (registered globally on the web group)
// closes /setup for good once an account exists.
...
// ─── Bank consent callback (unauthenticated by necessity) ─────────────────────
// The bank redirects the browser here after consent, and that request carries
// the bank's state parameter — not necessarily our session (a different browser,
// or a cookie dropped across the redirect chain). Requiring auth here breaks the
// consent flow in a way that only shows up against the real bank, so this route
// stays outside the auth group. It is safe: the controller acts solely on a
// state value it issued itself, and grants no access to any data.
Route::get('/banking/callback', BankingCallbackController::class)->name('banking.callback');
...
Route::middleware('auth')->group(function () {
    ...
    // The callback is declared above, outside this group — the bank cannot present
    // our session when it redirects.
