...
- **Status first, detail on click.** Integrations and long lists in Impostazioni render as `Settings/ConnectionRow.tsx`: closed it shows a coloured dot + one-line health summary + the primary action; the operational detail (tables, IBANs, sync errors) lives behind the chevron. A row in a non-`ok` tone opens by default — a broken connection is exactly when the detail is worth the space. Keep the frequent action (`Aggiorna ora`, `Sincronizza ora`) on the closed row so it never requires an expand.
- **Accordions over tabs for settings-style pages.** Impostazioni keeps every section on one scrollable page — `SectionHeading` groups (Connessioni / Dati / Categorie), each group a `Card` of collapsed `ConnectionRow`s. A tabbed variant was tried and rejected: hiding sections behind a switcher costs the single overview, which is the point of the page. Density comes from collapsing rows, not from splitting the page.
- **Responsive: `sm:`/`lg:` for page layout, a container query for a card, JS only for props.** A viewport breakpoint is right when the thing that changed is the *page* (the dashboard stops being a full-height flex column below `lg`). A container query is right when a component's space is set by its box rather than the window — the same card is narrow inside a 3-column grid at 1280px and inside a chat bubble at any width (`donut-layout`, `bubble-grid` in `app.css`). Do not reach for a viewport breakpoint to express a card's width: measured, a 390px phone gives the donut container ~20.9rem while a 1280px dashboard card with the sidebar open gives ~19.3rem — the phone is the *wider* container, so no `sm:` value can separate them. `useIsMobile()` (`lib/media.ts`) is the last resort, for what CSS cannot reach: Recharts sizes axes, legends and radii through props. It is `max-width: 639px`, one pixel under `sm`, so the JS and CSS switches flip on the same pixel — keep it that way.
