...
export default function Advisor({ configured, profile, goalObjective }: Props) {
    ...
    // The toast fires on a pending→done/failed transition, tracked via a ref so
    // it survives re-renders and isn't confused by a page that loads straight
    ...
    useEffect(() => {
        ...
        const tick = async () => {
            try {
                ...
                } else if (prev === 'pending' && data.status === 'done') {
                    setToast('Analisi completata.');
                } else if (prev === 'pending' && data.status === 'failed') {
    ...
    const generate = async () => {
        ...
        try {
            ...
            setStatus('pending');
            setReport(null);
            prevStatus.current = 'pending'; // so the next tick's done/failed is seen as a transition
